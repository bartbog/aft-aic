...
It is striking that many of the intuitions underlying about what ``good'' repairs are, such as minimality of change, are very similar to intuitions that have appeared in other domains of non-monotonic reasoning, such as for instance logic programming \mycite{LP} and default logic \mycite{DL}. 
...
As shown in~\cite{tase/Cruz-FilipeEGN13}, the intuitive semantics of founded repairs~\cite{iclp/CaropreseGSZ06} unexpectedly fails to respect the common-sense law of inertia, while the more restricted semantics of justified repairs~\cite{tplp/CaropreseT11} forbids natural repairs in some cases.
That work proposed an alternative operational semantics of well-founded repairs, which however is not modular~\cite{foiks/Cruz-Filipe14} and therefore is severely restricted in its practical applicability.
...
To be precise, we define semantics for AICs based on approximation fixpoint theory \cite{\refto{AFT}}, an abstract algebraic framework that unifies semantics of logic programming, default logic, autoepistemic logic \mycite{AEL}, abstract argumentation frameworks \cite{\refto{AF}} and abstract dialectical frameworks \mycite{ADF}, as shown by Denecker, Marek and Truszczy{\'n}ski [\citeyear{DeneckerMT00,DeneckerMT03}] and \citet{journals/ai/Strass13}. 
...
When this operator is defined, approximation fixpoint theory immediately provides us with \begin{inparaenum} \item a supported fixpoint semantics, \item a well-founded fixpoint semantics, \item a (partial) stable fixpoint semantics, \item a Kripke-Kleene fixpoint semantics, and \item a (partial) grounded fixpoint semantics. \end{inparaenum} 
