...
\longpaper{The problem of database repair has}\shortpaper{Database repairs have} been an important topic of research for \longpaper{more than thirty}\shortpaper{many} years~\cite{icdt/Abiteboul88}.
...
Among the most widely accepted criteria are minimality of change~\cite{Winslett90,ai/EiterG92} -- change as little as possible -- and the common-sense law of inertia~\cite{PrzymusinskiT97} -- do not change anything \longpaper{unless there is a reason to do it.}\shortpaper{without a reason to do it.}
...
In order to do so, we continue the work of \citet{iclp/Cruz-Filipe16}, who defined a semantic operator for AICs and used it to study the grounded fixpoint semantics \mycite{GroundedFixpoints} of AICs. 
In this paper, we define an approximating operator of Cruz-Filipe's semantic operator. 
From this \longpaper{approximating }operator, approximation fixpoint theory immediately provides us with \begin{inparaenum} 
...
\longpaper{Furthermore, we}\shortpaper{We} argue that\longpaper{, from a practical point of view,} the AFT-style well-founded semantics is \longpaper{very }valuable. 
Indeed, we show that this semantics can be computed in polynomial time, and that, on\longpaper{ a broad set of} practical examples, it corresponds to the intuitions underlying database repairs,  providing natural upper and lower bounds on the set of acceptable repairs (formally: the AFT-style well-founded model approximates all justified, stable and grounded repairs).

Besides defining a new class of interesting semantics, our work provides solid foundations for transferring \longpaper{a rich body of }work from the non-monotonic reasoning domain to AICs. For instance, we can now directly apply existing results from AFT, such as modularity results \cite{tocl/VennekensGD06,tocl/BogaertsVD16} or predicate introduction results \cite{VennekensMWD07a,VennekensMWD07b}. 
