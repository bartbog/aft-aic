In this section we summarize previous work on active integrity constraints (AICs), including results developed by~\cite{ppdp/FlescaGZ04,iclp/CaropreseGSZ06,tplp/CaropreseT11,tase/Cruz-FilipeEGN13}.
...
\luis{commented out disjunction: do we need it?}
...
\begin{example}
  In the setting of Example~\ref{ex:founded}, the only well-founded repair for $\fulldb$ is $\{{+c}\}$, as \ref{rf7}  is the only rule applicable to $\db$.
...
We are also interested in discussing the \emph{shifting property}.
