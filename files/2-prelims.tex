% \todo{REVIEWER 2: SHORTEN THIS SECTION!}
...
It follows from the informal explanation above that we expect normalization to preserve semantics. \shortpaper{T}{\longpaper{Indeed, t}his is the case for most semantics of AICs (the notorious exception being the semantics of justified repairs \cite{tplp/CaropreseT11}, which poses several other problems~\cite{tase/Cruz-FilipeEGN13}).
...
%  \end{compactitem}
% \end{definition}
% \bart{the ``but not conversely'' in this definition makes it a strict relation. That is somewhat unnatural to me. I would give the non-strict definition. All results remain to hold and precedence becomes a generalization of ``independent''. This is only a minor point.}
...
%transforming a database into an isomorphic one
...
%\todo{RVWR1: Preceding Def. 2.6, it is not clear to me what precisely the word "isomorphic" means in this context.}
...
Given a lattice $L$, approximation fixpoint theory (AFT) uses the bilattice 
...
AFT studies fixpoints of lattice operators $O:L\ra L$ through operators approximating $O$.
 An operator $A: L^2\to L^2$  is an \emph{approximator} of $O$ if it is \leqp-monotone,  and has the property that $O(x)\in A(x,x)$ for all $x$. %[x',y']$, where $(x',y')=A{x,x}$.
Approximators are
...
\shortpaper{All terminal well-founded inductions of $A$ have the $A$-well-founded fixpoint as limit \cite{lpnmr/DeneckerV07}.}
