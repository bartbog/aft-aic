...
It can be seen that $\UUU$ is consistent if and only if $\UUU(\alpha) \in \{\ltrue,\lfalse,\lunkn\}$ for all $\alpha$. 
...
% \bart{I REMOVED \suppin and \suppout!!!}
...
% \bart{It isn't literally the case. We can say something like
% 
...
% \luis{Ah. I added it after the informal explanation of the definition, I think it's more natural there. (I had started writing an explanation there earlier, but couldn't really see the point.)}
...
Secondly, we show that on two-valued update sets, $\Op$ and $\Ap$ coincide, i.e., that for all $\UU$, $\Op(\UU) =  \Ap(\UU)$. Take any $\alpha \in \acts$, we again prove this claim by a case analysis on the definition of $\Ap(\UU)(\alpha)$. 
\begin{itemize}
...
The terminology in the above definition uses ``repairs'' for certain classes of fixpoints of a semantic operator. It follows easily that  all two-valued update sets that are called ``repair'' in the above definition, indeed are repairs according to AIC terminology. 
...
Now, since $\UU$ is a repair, $(\UU,\UU)$ is a fixpoint of $\Ap$. Since $(\VV,\UU)\leqp(\UU,\UU)$, $\alpha \in \Ap(\VV,\UU)_1$ entails $\alpha \in \Ap(\UU,\UU)_1$, i.e., $\alpha \in \UU$. 
