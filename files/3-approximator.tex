...
of update actions.
Usually, we omit the bar, and simply write $\UU$ for the subset of \atoms as well. Only in places where the distinction between the two is essential (in technical parts of proofs) do we explicitly write $\overline \UU$. 
...
The lattice we are interested in is thus $\langle 2^{\atoms}, \subseteq\rangle$, where  smaller elements correspond to more preferred repairs. 
...
The well-founded semantics induced by AFT can in general differ from the existing well-founded semantics for AICs, as we show in Example \ref{ex:wf:wf}. To distinguish the two, we use the term AFT-well-founded semantics for the well-founded semantics induced by AFT. 
...
\begin{align*}
 ...
 \lnot a \land \lnot b \land \lnot c &\aicrule \add c
 \end{align*} with $\db = \emptyset$. Intuitively, we expect $\add c$ to be an element of ``good'' repairs, and (following the minimality of change principle), no other actions to be in ``good'' repairs. I
...
not hold. Consider for instance the following set $\aics$ of AICs
 ...
 However, also $a \mapsto \ltrue, b\mapsto \ltrue, c\mapsto \lfalse$ is a well-founded repair, obtained by applying first the second rule and then the first one.
 %(according to \citet{iclp/Cruz-Filipe16}). 
 %LCF: it's so simple to explain...
\end{example}
...
%  
\end{proof}
\luis{this one I still need to read closely}}
...
From existing result, we easily find how justified repairs and the AFT-well-founded repair relate. 
...
\end{corollary}
\luis{I have to think about this unipolar stuff. Does it make sense in the database world?}
\bart{I guess it would. You might never want to throw away information, only add things. Or in certain tables, only add, in other tables, only remove while repairing?}
