...
If $S$ is a subset of $L$, then $x$ is an \emph{upper bound} (respectively a \emph{lower bound}) of $S$ if for every $s\in S$, it holds that $s\leq x$ (resp.\ $x\leq s$).
...
A lattice $L$ is \emph{distributive} if $\land$ and $\lor$ distribute over each other, i.e.~$x\land(y\lor z)=(x\land y)\lor(x\land z)$ and $x\lor(y\land z)=(x\lor y)\land(x\lor z)$ for all $x,y,z\in L$.
...
\item The \emph{$A$-well-founded fixpoint} is the least precise partial $A$-stable fixpoint. \luis{maybe define ``least precise''? I'm guessing minimal wrt $\leqp$?}
