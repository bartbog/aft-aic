...
\begin{proof}
  ...
  \emph{(Hardness)}
  The proof is similar to the corresponding part of the proof of Proposition~\ref{prop:grounded-complexity}, using the fact that deciding whether a simple logic program has a grounded fixpoint is $\Sigma^P_2$-complete (Corollary~\ref{cor:simple-has-gf}).
...
\luis{not sure if this is whether we want the following text}

We now show that deciding whether a simple logic program has a grounded fixpoint is $\Sigma^P_2$-complete by establishing a reduction from the problem for arbitrary logic programs.
The intuition is simply to duplicate every atom $a$ as $a^b$ and $a^h$, to be used (only) in bodies or heads of rules, respectively, and add the necessary rules to ensure that they have the same interpretation in any fixpoint of the one-step consequence operator.
