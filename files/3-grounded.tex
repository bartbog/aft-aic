...
\begin{definition}\label{def:grounded}
 A set of update actions \UU is \emph{grounded} with respect to $\fulldb$ if, for each $\VV \subseteq  \UU$, there is a rule  $r\in \aics$ such that $\VV(\db)\models \body(r)$ and $\head(r)\in(\UU\setminus\VV)$. A \emph{grounded (weak) repair} is a (weak) repair that is grounded.
\end{definition}
%% \begin{definition}\label{def:grounded}
...
\begin{proof}
  ...
  The fact that $\UU$ is founded follows immediately from the definition of grounded repair, by letting $A$ range over the singleton sets of actions in $\UU$.
...
Also note that $\VV(\db)$ is a database for which the given set $\eta$ behaves very awkwardly: the only applicable AIC tells us to remove $b$, while the only possible repair is actually $\{{+a},{+c}\}$.
...
The discussion by~\cite[Section 8]{tplp/CaropreseT11} already proposes such a ``methodology'': choose a repair from the most restrictive category (justified, founded, or any).
...
This is due to the fact (already mentioned earlier) that we only consider \emph{normal} AICs in this paper. By Theorem~4 of~\cite{tplp/CaropreseT11}, all justified weak repairs are minimal when $\eta$ contains only normal AICs.
