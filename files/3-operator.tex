...
%% \[\left(\UU_1\uplus \UU_2\right)(\db) = \left(\UU_1\cup \UU_2\right) \setminus \{\alpha \mid \alpha,\alpha^D\in \UU_1\cup\UU_2\}\,.\]
\bart{Why does it say ``in this case, we also have the relation? Isn't it always equal (that's what was here previously)? 
\begin{align*} 
X \uplus Y  &:= X \cup (Y\setminus X^D) \setminus (X \cap Y^D) \\ 
   & = (X\setminus (X \cap Y^D) ) \cup  ((Y\setminus X^D)  \setminus (X \cap Y^D)) \\ 
   &= (X\setminus (Y^D) ) \cup  ((Y\setminus X^D)  \setminus (X \cap Y^D) ) \\ 
   &=  (X\setminus (Y^D) ) \cup (Y\setminus X^D)
\end{align*}
where the last equality holds since $Y$ and $Y^D$ are  disjoint.

Personally, I find the characterization 
\[\left(\UU_1\uplus \UU_2\right)(\db) = \left(\UU_1\cup \UU_2\right) \setminus \{\alpha \mid \alpha,\alpha^D\in \UU_1\cup\UU_2\}\,.\]
more understandably: take the union and restore consistency. 
Also: this definitions is nicely symmetrical.
}

\luis{Agh. I think I overcorrected.  The relation
\[\left(\UU_1\uplus \UU_2\right)(\db) = \left(\UU_1\cup \UU_2\right) \setminus \{\alpha \mid \alpha,\alpha^D\in \UU_1\cup\UU_2\}\]
always holds, but the previous property doesn't: if $\db=\emptyset$, $\UU_1=\{\remove a\}$ and $\UU_2=\{\add a\}$, then $\UU_2(\UU_1(\db))=\UU_2(\emptyset)=\{a\}\neq(\UU_1\uplus\UU_2)(\db)=\emptyset$.

I now changed the whole text.  Please delete these todos after you checked that you agree :-)
}
...
However, the translation defined in~\cite{Caroprese2011} is from logic programs to databases with AICs (rather than the other way around), so Proposition~\ref{lem:justified} is \emph{not} a direct consequence of those results.
