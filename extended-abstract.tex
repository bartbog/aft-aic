...
\input{idp-latex/idp-latex-no-theorem}
...
%
\author{Bart Bogaerts\inst{1,2}\orcidID{0000-0003-3460-4251} \and
Lu\'\i s Cruz-Filipe\inst{3}\orcidID{0000-0002-7866-7484} }
%
\authorrunning{B.\ Bogaerts and L.\ Cruz-Filipe}
% First names are abbreviated in the running head.
% If there are more than two authors, 'et al.' is used.
%
\institute{KU Leuven, Department of Computer Science, Leuven, Belgium 
% \\\email{bart.bogaerts@cs.kuleuven.be} 
\and
Vrije Universiteit Brussel, Department of Computer Science, Brussels, Belgium \and 
...
One of the key components of modern-day databases is the inclusion of integrity constraints: logical formulas that specify semantic relationships between the data being modeled.
...
It is striking that many intuitions about what ``good'' repairs are, such as the principle of minimality of change (change as little as possible), are similar to intuitions that surfaced in various domains of non-monotonic reasoning. %, such as logic programming and default logic . 
...
In the full version of this paper \mycite{AFT-AIC}, we use techniques that were developed for non-monotonic reasoning to solve the semantic problems of the field of active integrity constraints. 
...
% \emph{Grounded repairs} are natural counterparts to existing semantics in various non-monotonic reasoning domains such as logic programming; we discuss how they relate to other semantics for AICs.
We argue that \emph{grounded repairs} match our intuitions regarding AICs on a broad set of examples. 
% We studied properties of these new semantics. Some of them are existing semantics that are recovered using AFT; some are new, and relevant to the field.
% We showed that in particular the well-founded semantics induced by AFT and the grounded fixpoint semantics are very much in line with intuitions regarding AICs and satisfy several desirable properties. 
...
The contribution of this work goes beyond the definition of new semantics for AICs. 
...
For instance, we can now directly apply existing results from AFT, such as modularity results to AICs. 
It remains to be researched how these related for instance to existing modularity results for AICs. 
% Similarly, AFT provides us with predicate introduction results \cite{VennekensMWD07a,VennekensMWD07b}. 
Furthermore, our work paves the way to applying AFT to revision programming \cite{tplp/CaropreseT11}, and to AICs outside the database world \cite{ekaw/Cruz-Filipe16}.

\ignore{


This framework was initially developed by Denecker, Marek and Truszczy\'nski, henceforth referred to as DMT (\cite{DeneckerMT00}), after identifying analogies in the semantics of logic programming \mycite{LP}, autoepistemic logic (AEL) \cite{mo85} and default logic, hereafter abbreviated to DL \cite{ai/Reiter80}.
...
More recently, Strass \cite{journals/ai/Strass13} showed that AFT can also be used to characterize the major semantics of Dung's argumentation frameworks \mycite{AF} and abstract dialectical frameworks \mycite{ADF}.
...
\bibliography{extra-refs,idp-latex/krrlib.bib}
%
\end{document}
